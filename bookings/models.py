...
class RoomBooking(Booking):
    ...
    def clean(self):
        super().clean()
        if self.space and self.space.space_type != 'room':
            raise ValidationError("RoomBooking space must be of type 'room'.")
...
class HallBooking(Booking):
    ...
    def clean(self):
        super().clean()
        if self.space and self.space.space_type != 'hall':
            raise ValidationError("HallBooking space must be of type 'hall'.")
    ...
    @cached_property
    def hall_number(self):
        return self.space.space_number if self.space else None
